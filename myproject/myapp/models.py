...
from .cloudinary_storage import SmartVideoCloudinaryStorage
...
class SiteVisit(models.Model):
    ...
    def __str__(self):
        return f"{self.ip or 'anon'} · {self.device_type or 'Unknown'} · {self.last_seen:%Y-%m-%d %I:%M %p}"
...
class Service(TimeStampedModel):
    ...
    video = models.FileField(upload_to="services/", blank=True, null=True, storage=SmartVideoCloudinaryStorage())
...
class PortfolioVideo(models.Model):
    ...
    title = models.CharField(max_length=200, blank=True)
    video = models.FileField(upload_to="portfolio/videos/", blank=True, null=True, storage=SmartVideoCloudinaryStorage())
...
class Offer(TimeStampedModel):
    ...
    video = models.FileField(upload_to="offers/videos/", blank=True, null=True, storage=SmartVideoCloudinaryStorage())
    is_featured = models.BooleanField(default=False)
    is_active = models.BooleanField(default=True)
    sort_order = models.PositiveIntegerField(default=0)
